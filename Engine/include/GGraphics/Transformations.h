...
namespace GGraphics
{
   class Transformation
   {
	   public:
		...
		static GMath::Mat4f GetOrthographicProjectionMatrix(GGraphics::CanonicalViewVolume cvv)
		   {
			   GMath::Mat4f projectionMatrix{};

			   projectionMatrix = GMath::Mat4f::GetIdentityMatrix();
				projectionMatrix.mat[0][0] = 2 / cvv.width;
				projectionMatrix.mat[1][1] = 2 / cvv.height;
				projectionMatrix.mat[2][2] = 2 / (cvv.farDist - cvv.nearDist);
				projectionMatrix.mat[2][3] = (cvv.farDist + cvv.nearDist) / (cvv.farDist - cvv.nearDist);

			   return projectionMatrix;
		   }

		static GMath::Mat4f GetPerspectiveProjectionMatrix(GGraphics::CanonicalViewVolume cvv)
		{
		   GMath::Mat4f projectionMatrix{0};
		   projectionMatrix.mat[0][0] = (2 * cvv.nearDist) / cvv.width;
		   projectionMatrix.mat[1][1] = (2 * cvv.nearDist) / cvv.height;
		   projectionMatrix.mat[2][2] = ((cvv.nearDist + cvv.farDist) / (cvv.nearDist - cvv.farDist));
		   projectionMatrix.mat[2][3] = ((2 * cvv.nearDist * cvv.farDist) / (cvv.nearDist - cvv.farDist));
		   projectionMatrix.mat[3][2] = 1;
